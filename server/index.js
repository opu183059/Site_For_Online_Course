...
async function run() {
  try {
    ...
    const UsersData = client.db("CourseDB").collection("user");
    const ClassCollection = client.db("CourseDB").collection("class");
    const ClassBooking = client.db("CourseDB").collection("bookings");
...
app.get("/", (req, res) => {
  res.send(`coursera server is running on port: ${port}`);
});

app.listen(port, () => {
  console.log("coursea server is running");
});
